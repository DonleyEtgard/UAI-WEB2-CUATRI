import { useEffect } from "react";
import {
  Box,
  Container,
  Card,
  CardContent,
  CardHeader,
  Grid,
  Typography,
  Button,
  Chip,
} from "@mui/material";
import { useNavigate, useParams } from "react-router-dom";
import { useTranslation } from "react-i18next";
import { useProduct, useDeleteProduct } from "@/features/products";
import SkeletonLoader from "../../components/common/SkeletonLoader";
import EmptyState from "../../components/common/EmptyState";


const ProductDetailPage = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { t } = useTranslation();
  const { product, loading, reload } = useProduct(id || "");
  const { handleDelete: apiDelete } = useDeleteProduct();

  useEffect(() => {
    if (id) reload();
  }, [id, reload]);

  const handleDelete = async () => {
    if (!id) return;

    const ok = confirm(t("products.detail.confirmDelete"));
    if (!ok) return;

    try {
      await apiDelete(id);
      navigate("/app/products");
    } catch (err) {
      console.error("Error deleting product:", err);
      alert(t("products.detail.deleteError"));
    }
  };

  if (loading) return <SkeletonLoader count={4} height={28} />;
  if (!product) return <div className="p-6"><EmptyState title={t("products.detail.notFound")} description={t("products.detail.notFoundDescription")} /></div>;

  return (
  <Box sx={{ minHeight: "100vh", py: { xs: 2, md: 4 } }}>
    <Container maxWidth="lg">

      {/* HEADER */}
      <Box
        sx={{
          display: "flex",
          justifyContent: "space-between",
          alignItems: "flex-end",
          mb: 4,
          pb: 3,
          borderBottom: "2px solid #e0e7ff",
        }}
      >
        <Box>
          <h1
            style={{
              fontSize: "2.25rem",
              fontWeight: 900,
              margin: 0,
              color: "#fff",
            }}
          >
            {product.name}
          </h1>

          <p
            style={{
              fontSize: "0.75rem",
              color: "#64748b",
              marginTop: "0.5rem",
              fontWeight: 600,
              letterSpacing: "0.2em",
              textTransform: "uppercase",
            }}
          >
            {t("products.detail.subtitle")}
          </p>
        </Box>

        <Chip
          label={product.isActive ? t("products.detail.active") : t("products.detail.hidden")}
          color={product.isActive ? "success" : "default"}
        />
      </Box>

      {/* MAIN IMAGE */}
      {product.images && product.images.length > 0 && (
        <Box sx={{ mb: 4, display: 'flex', justifyContent: 'center' }}>
          <Box
            component="img"
            src={product.images[0]}
            alt={product.name}
            sx={{ width: '100%', maxHeight: 400, objectFit: 'contain', borderRadius: 4, bgcolor: 'background.paper' }}
          />
        </Box>
      )}

      {/* KPI CARDS */}
      <Grid container spacing={2} sx={{ mb: 4 }}>
        <Grid size={{ xs: 12, md: 4 }}>
          <Card>
            <CardContent>
              <Typography variant="overline">
                {t("products.detail.salePrice")}
              </Typography>

              <Typography
                variant="h4"
                sx={{ fontWeight: 900, color: 'primary.main' }}
              >
                ${product.price}
              </Typography>
            </CardContent>
          </Card>
        </Grid>

        <Grid size={{ xs: 12, md: 4 }}>
          <Card>
            <CardContent>
              <Typography variant="overline">
                {t("products.detail.cost")}
              </Typography>

              <Typography
                variant="h4"
                sx={{ fontWeight: 900 }}
              >
                ${product.cost || 0}
              </Typography>
            </CardContent>
          </Card>
        </Grid>

        <Grid size={{ xs: 12, md: 4 }}>
          <Card>
            <CardContent>
              <Typography variant="overline">
                {t("products.detail.availableStock")}
              </Typography>

              <Typography
                variant="h4"
                sx={{ fontWeight: 900, color: product.stock > 0 ? "success.main" : "error.main" }}
              >
                {product.stock}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
      </Grid>

      {/* DETALLE */}
      <Card sx={{ mb: 4 }}>
        <CardHeader
          title={t("products.detail.generalInfo")}
          subheader={t("products.detail.generalInfoDescription")}
        />

        <CardContent>
          <Grid container spacing={3}>

            <Grid size={{ xs: 12, md: 6 }}>
              <Typography variant="overline">
                {t("products.detail.category")}
              </Typography>

              <Typography variant="h6">
                {(typeof product.category === "object"
                  ? product.category?.name
                  : product.category) || t("products.detail.noCategory")}
              </Typography>
            </Grid>

            <Grid size={{ xs: 12, md: 6 }}>
              <Typography variant="overline">
                {t("products.detail.status")}
              </Typography>

              <Box sx={{ mt: 1 }}>
                <Chip
                  label={
                    product.isActive
                      ? "Activo"
                      : t("products.detail.hidden")
                  }
                  color={
                    product.isActive
                      ? "success"
                      : "default"
                  }
                />
              </Box>
            </Grid>

            <Grid size={12}>
              <Typography variant="overline">
                {t("products.detail.description")}
              </Typography>

              <Typography
                variant="body1"
                sx={{
                  mt: 1,
                  color: "text.secondary",
                }}
              >
                {product.description ||
                  t("products.detail.noDescription")}
              </Typography>
            </Grid>
          </Grid>
        </CardContent>
      </Card>

      {/* ACTIONS */}
      <Box
        sx={{
          display: "flex",
          gap: 2,
          flexWrap: "wrap",
          color: 'white',
        }}
      >
        <Button
          variant="outlined"
          onClick={() =>
            navigate("/app/products")
          }
        >
          {t("products.detail.back")}
        </Button>

        <Button
  variant="outlined"
  sx={{
    color: "white",
    borderColor: "white",
    fontSize: 14,
    fontWeight: 700,
    "&:hover": {
      borderColor: "white",
      backgroundColor: "rgba(255,255,255,0.08)",
    },
  }}
  onClick={() =>
    navigate(`/app/products/edit/${id}`)
  }
>
  {t("products.detail.edit")}
</Button>

        <Button
          variant="contained"
          color="error"
          onClick={handleDelete}
        >
          {t("products.detail.delete")}
        </Button>
      </Box>

    </Container>
  </Box>
);
};
export default ProductDetailPage;