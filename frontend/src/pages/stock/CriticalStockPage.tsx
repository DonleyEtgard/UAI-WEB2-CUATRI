...
const CriticalStockPage = () => {
  ...
  const getAlertConfig = (stock: number) => {
    if (stock === 0) {
      return { label: t("stock.critical.outOfStock"), color: "error" as const, icon: ErrorIcon, severity: "high" };
    }
    if (stock <= 5) {
      return { label: t("stock.critical.critical"), color: "warning" as const, icon: WarningIcon, severity: "critical" };
    }
    return { label: t("stock.critical.low"), color: "info" as const, icon: CheckCircleIcon, severity: "low" };
  };
    ...
    <Box sx={{ minHeight: '100vh', py: { xs: 2, md: 4 }, px: { xs: 2, md: 0 } }}>
      <Container maxWidth="lg">
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 4, pb: 3, borderBottom: '2px solid #e0e7ff' }}>
          <Box>
            <h1 style={{ fontSize: '2rem', fontWeight: 900, margin: 0 }}>{t("stock.critical.title")}</h1>
            <p style={{ fontSize: '0.875rem', color: '#64748b', marginTop: '0.5rem', fontWeight: 600 }}>{t("stock.critical.subtitle")}</p>
          </Box>
          <Button 
        ...
        {loading ? (
          ...
          <EmptyState 
            title={t("stock.critical.healthyInventory")} 
            description={t("stock.critical.healthyInventoryDescription")}
          />
        ) : (
                  ...
                  <Card sx={{ borderRadius: 3, boxShadow: 2, transition: 'all 0.3s ease', '&:hover': { boxShadow: 4, transform: 'translateY(-4px)' } }}>
                    <CardContent sx={{ p: 3 }}>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderTop: '1px solid #e0e7ff', pt: 2 }}>
                        <Box>
                          <Typography variant="caption" sx={{ color: '#64748b', fontWeight: 600, display: 'block', mb: 0.5 }}>{t("stock.critical.stock")}</Typography>
