...
const SaleDetailPage = () => {
  ...
  const handleDelete = async () => {
    ...
    const ok = confirm(t("sales.detail.confirmDelete"));
    if (!ok) return;
    try {
      setDeleting(true);
  ...
  if (!sale || !metrics) return (
    <Box sx={{ display: 'flex', justifyContent: 'center', py: 10, minHeight: '100vh' }}>
      <EmptyState title={t("sales.detail.notFound")} description={t("sales.detail.notFoundDescription")} />
    </Box>
  );
    ...
    <Box sx={{ minHeight: '100vh', py: { xs: 2, md: 4 }, px: { xs: 2, md: 0 } }}>
      <Container maxWidth="lg">
        ...
        <Card sx={{ borderRadius: 3, boxShadow: 3, mb: 4, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white' }}>
          <CardContent sx={{ p: { xs: 3, md: 4 } }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
              <Box> 
                <Typography variant="h4" sx={{ fontWeight: 800 }}>{t("sales.detail.sale")} #{sale._id.slice(-8)}</Typography>
        ...
        <Grid container spacing={3} sx={{ mb: 4 }}>
          ...
          <Grid size={{ xs: 12, md: 8 }}>
            <Card sx={{ borderRadius: 3, boxShadow: 2 }}>
              <CardHeader title={t("sales.detail.saleInfo")} />
              <CardContent>
                <Grid container spacing={3}>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="caption" sx={{ color: '#64748b', fontWeight: 600, display: 'block', textTransform: 'uppercase', mb: 0.5 }}>{t("sales.detail.total")}</Typography>
                      <Typography variant="h5" sx={{ fontWeight: 700, color: '#667eea' }}>${sale.total.toFixed(2)}</Typography>
                    </Box>
                  </Grid>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="caption" sx={{ color: '#64748b', fontWeight: 600, display: 'block', textTransform: 'uppercase', mb: 0.5 }}>{t("sales.detail.paid")}</Typography>
                      <Typography variant="h5" sx={{ fontWeight: 700, color: '#10b981' }}>${sale.amountPaid.toFixed(2)}</Typography>
                    </Box>
                  </Grid>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="caption" sx={{ color: '#64748b', fontWeight: 600, display: 'block', textTransform: 'uppercase', mb: 0.5 }}>{t("sales.detail.change")}</Typography>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Card sx={{ borderRadius: 3, boxShadow: 2, mb: 2 }}>
              <CardContent>
                <Typography variant="caption" sx={{ color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>{t("sales.detail.subtotal")}</Typography>
                <Typography variant="h5" sx={{ fontWeight: 700, mt: 1 }}>${metrics.subtotal.toFixed(2)}</Typography>
              </CardContent>
            </Card>
            <Card sx={{ borderRadius: 3, boxShadow: 2, mb: 2 }}>
              <CardContent>
                <Typography variant="caption" sx={{ color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>{t("sales.detail.averageItem")}</Typography>
                <Typography variant="h5" sx={{ fontWeight: 700, mt: 1 }}>${metrics.avgItem.toFixed(2)}</Typography>
              </CardContent>
            </Card>
            <Card sx={{ borderRadius: 3, boxShadow: 2 }}>
              <CardContent>
                <Typography variant="caption" sx={{ color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>{t("sales.detail.estimatedProfit")}</Typography>
        ...
        <Card sx={{ borderRadius: 3, boxShadow: 2, mb: 4 }}>
          <CardHeader title={t("sales.detail.products")} subheader={t("sales.detail.items", { count: sale.items.length })} />
          <TableContainer>
            <Table>
              <TableHead>
                <TableRow sx={{ bgcolor: '#f8fafc' }}>
                  <TableCell><strong>{t("sales.detail.product")}</strong></TableCell>
                  <TableCell align="center"><strong>{t("sales.detail.quantity")}</strong></TableCell>
                  <TableCell align="right"><strong>{t("sales.detail.unitPrice")}</strong></TableCell>
                  <TableCell align="right"><strong>{t("sales.detail.total")}</strong></TableCell>
                </TableRow>
              </TableHead>
             <TableBody>
  {sale.items?.map((item, idx) => (
    ...
    >
      <TableCell>
        {item.productName || item.product || t("sales.detail.placeholderProduct")}
      </TableCell>
...
>
  ...
  >
     {deleting ? <CircularProgress size={20} /> : t("sales.detail.delete")}
    </Button>
   </Box>
      </Container>
    </Box>
