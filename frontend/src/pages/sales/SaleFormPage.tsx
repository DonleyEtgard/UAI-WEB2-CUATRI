...
const SaleFormPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCustomerId || cart.length === 0) return alert(t("sales.form.completeData"));
    ...
    try {
     ...
     await API.post("/sales", {
      ...
      alert(t("sales.form.processError"));
    } finally {
      setLoading(false);
    }
  };
        ...
        <Grid container spacing={4}>
          ...
          <Grid size={{ xs: 12, md: 8 }}>
            <Card sx={{ borderRadius: 3, boxShadow: 3 }}>
              <CardHeader 
                title={t("sales.form.newTransaction")} 
                subheader={t("sales.form.subtitle")}
              ...
              <CardContent sx={{ pt: 4 }}>
                ...
                <Box sx={{ mb: 4 }}>
                  <FormControl fullWidth>
                    <InputLabel>{t("sales.form.customer")}</InputLabel>
                    <Select
                      value={selectedCustomerId}
                      onChange={(e) => setSelectedCustomerId(e.target.value)}
                      label={t("sales.form.customer")}
                    >
                      <MenuItem value="">{t("sales.form.selectCustomer")}</MenuItem>
                      {customers.map(c => (
                ...
                <Box sx={{ mb: 4 }}>
                  <TextField
                    fullWidth
                    label={t("sales.form.searchProducts")}
                    placeholder={t("sales.form.searchPlaceholder")}
                    value={searchQuery}
                ...
                <Grid container spacing={2}>
                  {searchQuery && filteredProducts.length > 0 ? (
                          ...
                          <CardContent>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                              <Box>
                                ...
                                <Typography variant="caption" sx={{ color: '#64748b' }}>{t("sales.form.stock")}: {p.stock}</Typography>
                              </Box>
                    ...
                    <Grid size={12}>
                      <Typography sx={{ textAlign: 'center', color: '#94a3b8', py: 4 }}>{t("sales.form.noMatches")}</Typography>
                    </Grid>
                  ) : (
                    <Grid size={12}>
                      <Typography sx={{ textAlign: 'center', color: '#94a3b8', py: 4 }}>{t("sales.form.typeToSearch")}</Typography>
                    </Grid>
                  )}
                </Grid>
              </CardContent>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Card sx={{ borderRadius: 3, boxShadow: 3, display: 'flex', flexDirection: 'column', height: '100%' }}>
              <CardHeader 
                title={t("sales.form.saleSummary")}
                subheader={t("sales.form.items", { count: cart.length })}
                sx={{ pb: 2 }}
              />
              <CardContent sx={{ flexGrow: 1, overflowY: 'auto', maxHeight: '400px' }}>
                {cart.length === 0 ? (
                  <Box sx={{ textAlign: 'center', py: 4, color: '#94a3b8' }}>
                    ...
                    <Typography>{t("sales.form.emptyCart")}</Typography>
                  </Box>
                ) : (
                  <TableContainer>
                    <Table size="small">
              ...
              <Box sx={{ p: 2, borderTop: '1px solid #e0e7ff', bgcolor: '#f8fafc' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography variant="body2" sx={{ color: '#64748b' }}>{t("sales.form.subtotal")}:</Typography>
                  <Typography variant="body2">${total.toFixed(2)}</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                  <Typography variant="body2" sx={{ color: '#64748b' }}>{t("sales.form.taxes")}:</Typography>
                  <Typography variant="body2">$0.00</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3, pt: 1, borderTop: '1px solid #e0e7ff' }}>
                  <Typography variant="h6" sx={{ fontWeight: 700 }}>{t("sales.form.total")}:</Typography>
                ...
                <Box sx={{ display: 'flex', gap: 2, flexDirection: 'column' }}>
                  ...
                  >
                    {loading ? <CircularProgress size={24} /> : t("sales.form.finishSale")}
                  </Button>
                  <Button 
                    fullWidth
                    variant="outlined"
