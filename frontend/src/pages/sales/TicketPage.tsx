...
const TicketPage = () => {
 ...
 const sendWhatsApp = async () => {
  try {
    ...
    console.error(err);
    alert(t("sales.ticket.sendWhatsappError"));
  }
};
  ...
  const sendTelegram = async () => {
    try {
      ...
      console.error(err);
      alert(t("sales.ticket.sendTelegramError"));
    }
  };
  ...
  if (!ticket) return (
    <Box sx={{ textAlign: 'center', py: 10 }}>
      <Typography>{t("sales.ticket.notFound")}</Typography>
    </Box>
  );
      ...
      <Container maxWidth="sm">
        <Card sx={{ borderRadius: 3, boxShadow: 3, bgcolor: '#ffe0ab' }}>
          <CardContent sx={{ textAlign: 'center', pt: 4 }}>
            ...
            <Typography variant="h4" sx={{ fontWeight: 700, mb: 2 }}>{t("sales.ticket.title")}</Typography>
          ...
          <CardContent sx={{ py: 2 }}>
            <Box sx={{ display: 'grid', gap: 2, fontSize: 14 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography sx={{ color: '#64748b' }}>{t("sales.ticket.customer")}:</Typography>
                <Typography sx={{ fontWeight: 600 }}>{sale.customer?.name || t("sales.ticket.finalConsumer")}</Typography>
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography sx={{ color: '#64748b' }}>{t("sales.ticket.seller")}:</Typography>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography sx={{ color: '#64748b' }}>{t("sales.ticket.date")}:</Typography>
          ...
          <CardContent sx={{ py: 2 }}>
            <Box sx={{ display: 'grid', gap: 1.5, fontSize: 14 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700 }}>
                <Typography sx={{ fontWeight: 700 }}>{t("sales.ticket.total")}:</Typography>
              ...
              {sale.paymentMethod === "cash" && (
                <>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography sx={{ color: '#64748b' }}>{t("sales.ticket.paid")}:</Typography>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', color: '#10b981', fontWeight: 600 }}>
                    <Typography sx={{ color: '#10b981' }}>{t("sales.ticket.change")}:</Typography>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <Typography sx={{ color: '#64748b' }}>{t("sales.ticket.method")}:</Typography>
                <Typography sx={{ fontWeight: 600, textTransform: 'capitalize' }}>{sale.paymentMethod}</Typography>
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <Typography sx={{ color: '#64748b' }}>{t("sales.ticket.status")}:</Typography>
          ...
          {sale.notes && (
            <>
              <Divider />
              <CardContent sx={{ py: 2 }}>
                <Typography variant="caption" sx={{ color: '#64748b' }}>{t("sales.ticket.note")}:</Typography>
